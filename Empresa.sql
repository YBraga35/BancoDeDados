/*
Scripts do livro ELMASRI, Ramez; NAVATHE, Shamkant B. Sistema de bancos de dados. 6. ed. São Paulo: Pearson, 2011-2014. 788 p.
*/
create schema Empresa;
use Empresa;

create table departamento (
  dnome char(20) not null,
  dnumero numeric(1) not null,  
  cpf_gerente numeric(9) not null,   
  datainiciogerente date not null, 
  primary key (dnumero), 
  unique (dnome)
);

create table funcionario (
  pnome char(10) not null,
  unome char(20) not null,
  cpf numeric(9) not null,  
  datanascimento date not null,
  endereco char(30) not null,
  sexo char(1) not null,
  salario numeric(5) not null,
  cpfsupervisor numeric(9),
  dnumero numeric(1) not null, 
  primary key (cpf), 
  foreign key (dnumero) references departamento(dnumero)
); 

create table projeto (
  projnome char(20) not null,
  projnumero numeric(2) not null,  
  projlocal char(20) not null,
  dnumero numeric(1) not null, 
  primary key (projnumero), 
  unique (projnome), 
  foreign key (dnumero) references departamento(dnumero)
); 

create table trabalha_em (
  cpf numeric(9) not null,  
  projnumero numeric(2) not null, 
  horas numeric(5,1),  
  primary key (cpf, projnumero), 
  foreign key (cpf) references funcionario(cpf),
  foreign key (projnumero) references projeto(projnumero)
); 

create table dependente (
  cpf numeric(9) not null,  
  dependente_name char(10) not null,
  sexo char(1) not null,
  datanascimento date not null,
  parentesco char(30) not null,
  primary key (cpf, dependente_name),
  foreign key (cpf) references funcionario(cpf)
); 

create table local_dept (
  dnumero numeric(1) not null,
  dlocalizacao char (15) not null,
  primary key (dnumero, dlocalizacao), 
  foreign key (dnumero) references departamento(dnumero)
);

insert into departamento values 
 ('Research',5,333445555,'1988-05-22'),
 ('Administration',4,987654321,'1995-01-01'),
 ('Headquarters',1,888665555,'1981-06-19');

insert into funcionario values 
 ('John','Smith',123456789,'1965-01-09','731 Fondren, Houston TX','M',30000,333445555,5),
 ('Franklin','Wong',333445555,'1965-12-08','638 Voss, Houston TX','M',40000,888665555,5),
 ('Alicia','Zelaya',999887777,'1968-01-19','3321 Castle, Spring TX','F',25000,987654321,4),
 ('Jennifer','Wallace',987654321,'1941-06-20','291 Berry, Bellaire TX','F',43000,888665555,4),
 ('Ramesh','Narayan',666884444,'1962-09-15','975 Fire Oak, Humble TX','M',38000,333445555,5),
 ('Joyce','English',453453453,'1972-07-31','5631 Rice, Houston TX','F',25000,333445555,5),
 ('Ahmad','Jabbar',987987987,'1969-03-29','980 Dallas, Houston TX','M',25000,987654321,4),
 ('James','Borg',888665555,'1937-11-10','450 Stone, Houston TX','M',55000,null,1);

insert into projeto values 
 ('ProductX',1,'Bellaire',5),
 ('ProductY',2,'Sugarland',5),
 ('ProductZ',3,'Houston',5),
 ('Computerization',10,'Stafford',4),
 ('Reorganization',20,'Houston',1),
 ('Newbenefits',30,'Stafford',4);

insert into trabalha_em values 
 (123456789,1,32.5),
 (123456789,2,7.5),
 (666884444,3,40.0),
 (453453453,1,20.0),
 (453453453,2,20.0),
 (333445555,2,10.0),
 (333445555,3,10.0),
 (333445555,10,10.0),
 (333445555,20,10.0),
 (999887777,30,30.0),
 (999887777,10,10.0),
 (987987987,10,35.0),
 (987987987,30,5.0),
 (987654321,30,20.0),
 (987654321,20,15.0),
 (888665555,20,null);

insert into dependente values 
 (333445555,'Alice','F','1986-04-04','Daughter'),
 (333445555,'Theodore','M','1983-10-25','Son'),
 (333445555,'Joy','F','1958-05-03','Spouse'),
 (987654321,'Abner','M','1942-02-28','Spouse'),
 (123456789,'Michael','M','1988-01-04','Son'),
 (123456789,'Alice','F','1988-12-30','Daughter'),
 (123456789,'Elizabeth','F','1967-05-05','Spouse');

insert into local_dept values
 (1,'Houston'),
 (4,'Stafford'),
 (5,'Bellaire'),
 (5,'Sugarland'),
 (5,'Houston');

alter table departamento 
 add constraint depemp foreign key (cpf_gerente) references funcionario(cpf);

alter table funcionario   
 add constraint empemp foreign key (cpfsupervisor) references funcionario(cpf);